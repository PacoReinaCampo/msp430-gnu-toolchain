...
{
  static bfd_boolean warned[2][4] = {{FALSE, FALSE, FALSE, FALSE}, {FALSE, FALSE, FALSE, FALSE}};
  int i = WARN_LOWER;
...
/* Return TRUE if a non-debug input section with positive size matches the
   given name.  */
static int
...
gld${EMULATION_NAME}_finish (void)
{
  ...
  /* .{preinit,init,fini}_array might be in .rodata or .rodata2.  */
  ...
  /* Pre-2019 MSP430 linker scripts distributed by TI do not include ALIGN
     directives to ensure __preinit_array_start, __init_array_start or
     __fini_array_start are word aligned.
     __init_array_start is really the only one that matters, since
     .preinit_array is unlikely to contain anything, and .fini_array is not
     likely to get run at all.
     If __init_array_start is not word aligned, the code to run through the
     array and call the constructors will crash.  */
  if (((rodata_sec && input_section_exists (rodata_sec->children.head,
					   ".init_array"))
       || (rodata2_sec && input_section_exists (rodata2_sec->children.head,
					       ".init_array")))
      && (sym = bfd_link_hash_lookup (link_info.hash, "__init_array_start",
				      FALSE, FALSE, TRUE))
      && sym->type == bfd_link_hash_defined
      && sym->u.def.value % 2)
    {
      einfo ("%P: warning: \"__init_array_start\" symbol (%U) "
	     "is not word aligned\n", NULL);
    }
  if (((rodata_sec && input_section_exists (rodata_sec->children.head,
					   ".preinit_array"))
       || (rodata2_sec && input_section_exists (rodata2_sec->children.head,
					       ".preinit_array")))
      && (sym = bfd_link_hash_lookup (link_info.hash, "__preinit_array_start",
				      FALSE, FALSE, TRUE))
      && sym->type == bfd_link_hash_defined
      && sym->u.def.value % 2)
    {
      einfo ("%P: warning: \"__preinit_array_start\" symbol (%U) "
	     "is not word aligned\n", NULL);
    }
  if (((rodata_sec && input_section_exists (rodata_sec->children.head,
					   ".fini_array"))
       || (rodata2_sec && input_section_exists (rodata2_sec->children.head,
					       ".fini_array")))
      && (sym = bfd_link_hash_lookup (link_info.hash, "__fini_array_start",
				      FALSE, FALSE, TRUE))
      && sym->type == bfd_link_hash_defined
      && sym->u.def.value % 2)
    {
      einfo ("%P: warning: \"__fini_array_start\" symbol (%U) "
	     "is not word aligned\n", NULL);
    }
}
