...
static bfd_boolean
elf32_msp430_merge_msp430_attributes (bfd *ibfd, bfd *obfd)
{
  obj_attribute *in_attr, *in_gnu_attr;
  obj_attribute *out_attr, *out_gnu_attr;
  ...
  if ((in_attr[OFBA_MSPABI_Tag_Code_Model].i
      ...
      && (in_attr[OFBA_MSPABI_Tag_Data_Model].i
	  == OFBA_MSPABI_Val_Data_Model_LARGE))
    {
      /* In 8.2.0.52 we (incorrectly) defined the lower region attribute as an
	 mspabi attribute with tag number 10 and the following values.
	 To maintain compatibility we must handle these as well.
	 This means we are not compatible with the OFBA_MSPABI_Tag_enum_size
	 mspabi attribute yet, since the tag values overlap.  */
      enum {
	  _OLD_MSP_REGION_VAL_LOWER = 1,
	  _OLD_MSP_REGION_VAL_UPPER = 2,
	  _OLD_MSP_REGION_TAG =  10,
      };

      /* We cannot allow "lower region only" to be linked with any other
	 values, if we are using the large code/data model.
	...
	= (in_gnu_attr[Tag_GNU_MSP430_DATA_REGION].i == Val_GNU_MSP430_DATA_REGION_LOWER
	   || in_attr[_OLD_MSP_REGION_TAG].i == _OLD_MSP_REGION_VAL_LOWER);
      /* The output BFD should not ever use the _OLD_MSP_REGION_TAG, but for
         now we allow it and emit a warning if it does.  This warning will
	 conflict with OFBA_MSPABI_Tag_enum_size if that gets implemented in
	 GCC/GAS.  */
      bfd_boolean obfd_lower_region_used
	= (out_gnu_attr[Tag_GNU_MSP430_DATA_REGION].i == Val_GNU_MSP430_DATA_REGION_LOWER
	   || out_attr[_OLD_MSP_REGION_TAG].i == _OLD_MSP_REGION_VAL_LOWER);
      if (out_attr[_OLD_MSP_REGION_TAG].i == _OLD_MSP_REGION_VAL_LOWER
	  || out_attr[_OLD_MSP_REGION_TAG].i == _OLD_MSP_REGION_VAL_UPPER)
	{
	  _bfd_error_handler
	    (_("warning: unexpected mspabi attribute %d in output bfd %B."),
	     _OLD_MSP_REGION_TAG, obfd);
	}
      if (ibfd_lower_region_used != obfd_lower_region_used)
	{
	  _bfd_error_handler
	    (_("error: %B uses the upper region for data, but %B assumes data is in lower memory."),
...
#undef max
  ...
  return elf32_msp430_merge_msp430_attributes (ibfd, obfd);
}
